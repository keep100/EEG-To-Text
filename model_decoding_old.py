...
class BrainTranslator(nn.Module):
    def __init__(self, pretrained_layers, in_feature = 840, decoder_embedding_size = 1024, additional_encoder_nhead=8, additional_encoder_dim_feedforward = 2048):
        ...
        gru_hidden_size=512
        num_gru_layers=8
        # self.gru = nn.GRU(in_feature, gru_hidden_size, batch_first=True)
        # self.fc_gru = nn.Linear(gru_hidden_size, in_feature)
        self.gru_layers = nn.ModuleList([nn.GRU(in_feature if i == 0 else gru_hidden_size, gru_hidden_size, batch_first=True) for i in range(num_gru_layers)])
        self.fc_gru = nn.Linear(gru_hidden_size, in_feature)
    ...
    def forward(self, input_embeddings_batch, input_masks_batch, input_masks_invert, target_ids_batch_converted):
        ...
        for gru_layer in self.gru_layers:
            gru_output, _ = gru_layer(input_embeddings_batch)
            input_embeddings_batch=gru_output
        input_embeddings_batch = F.relu(self.fc_gru(input_embeddings_batch))
